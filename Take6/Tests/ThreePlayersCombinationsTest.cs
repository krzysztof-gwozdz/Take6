...
internal class ThreePlayersCombinationsTest
{
    private const long NumberOfTests = 100;
    ...
    private IEnumerable<Player> AllPlayers => _players1.Concat(_players2).Concat(_players3);

    private readonly (Player player1, Player player2, Player player3)[] _playersCombinations;

    public ThreePlayersCombinationsTest()
    {
    ...
    private void Test()
    {
        foreach (var (player1, player2, player3) in _playersCombinations)
            for (var i = 0; i < NumberOfTests; i++)
                new Game(new[] { player1, player2, player3 }).Play();
    }

    private void DisplayResults()
    {
        var numberOfGames = AllPlayers.First().GameResults.Count * 2;
        Console.WriteLine($"Results for 3 players after {numberOfGames} games: ");
